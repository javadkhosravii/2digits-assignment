...
import { fetchBlogBySlug, fetchRelatedBlogs } from '@/server/prepr';
...
export default async function BlogDetail({ params }: Props) {
  ...
  // Fetch related blogs based on the first category
  const relatedBlogs =
    blog.categories.length > 0 && blog.categories[0]?.slug
      ? await fetchRelatedBlogs(blog.categories[0].slug, blog._id)
      : [];
              ...
              <div className="flex flex-wrap gap-2">
                {blog.categories.map((category) => (
                  <span
                    ...
                    className="w-fit rounded bg-[#E9EBF4] px-3 py-1 text-xs font-semibold text-black sm:text-sm">
                    {category.body?.toUpperCase()}
                  </span>
                ))}
              </div>
            )}
